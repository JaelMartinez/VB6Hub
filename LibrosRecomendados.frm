...
Begin VB.Form LibrosRecomendados 
   Caption         =   "LibrosRecomendados"
   ClientHeight    =   8010
   ClientLeft      =   5220
   ClientTop       =   3300
   ClientWidth     =   19245
   ...
   End
End
Attribute VB_Name = "LibrosRecomendados"
...
Private Sub cmdEliminar_Click()
    ...
    respuesta = MsgBox("¿Está seguro de que desea eliminar el libro '" & TituloLibro & "' de la lista de libros recomendados?", vbYesNo + vbQuestion, "Confirmar Eliminación")
    
    If respuesta = vbYes Then
        ' Eliminar el libro de la base de datos
        Dim sql As String
        sql = "DELETE FROM LibrosRecomendados WHERE LibroID IN (SELECT LibroID FROM Libros WHERE Titulo = '" & TituloLibro & "')"
        conn.Execute sql
...
Private Sub Form_Load()
    ...
    ' Cargar los libros recomendados en el ListView
    Call LoadRecomendadosIntoListView
End Sub

Private Sub LoadRecomendadosIntoListView()
    ' Conectar a la base de datos y llenar el ListView con los libros recomendados
    Set rs = New ADODB.Recordset
    rs.Open "SELECT L.Titulo, L.Autor, L.Genero, L.Calificacion, L.Sinopsis FROM Libros L INNER JOIN LibrosRecomendados LR ON L.LibroID = LR.LibroID", conn, adOpenStatic, adLockReadOnly
